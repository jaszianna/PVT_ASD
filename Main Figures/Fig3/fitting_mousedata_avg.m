...
fPath = 'h:\work\KOKI_mouse_sleep\ÁBRA_20221111\Inhibited SwiChR\PRE\'; % set the path ot the input folder (which contains all individual spectum files in .dat)
outfolder = 'h:\work\KOKI_mouse_sleep\ÁBRA_20221111\Inhibited SwiChR\PRE\'; % output folder for figures and .xlsx files
...
close all;
%% parameters require user input
fPath = 'h:\work\KOKI_mouse_sleep\ÁBRA_20221111\Inhibited SwiChR\POST\'; % set the path ot the input folder (which contains all individual spectum files in .dat)
outfolder = 'h:\work\KOKI_mouse_sleep\ÁBRA_20221111\Inhibited SwiChR\POST\'; % output folder for figures and .xlsx files
% addpath('h:\work\KOKI_mouse_sleep\'); % set the forder where the script is located, or add script manually to the path (then, this line is not required)
electrodeidx = [1]; % set the number of electores
electrode_names = {'Fr'}; % set electrode labels (required for the output only)
% set the text you want to write in first raw of the output .xlsx file in
...
meanspectra2 = nanmean(spectra_nrem_mean,2);
p_mean2 = mean(p_mpeak_mean);
...
%             saveas(gcf, [outfolder, 'fig_avg.tif']);
 hold on
            ...
            plot(log(f1:freqsteps:f4),log((meanspectra2((f1/freqsteps+1):(f4/freqsteps+1),electrodeidx(elec)))),'k');
            hold on
            plot(log(f1:freqsteps:f2),polyval(p_mean2,log(f1:freqsteps:f2)),'g');
            plot(log(f3:freqsteps:f4),polyval(p_mean2,log(f3:freqsteps:f4)),'g');
            plot(log(f2:freqsteps:f3),polyval(p_mean2,log(f2:freqsteps:f3)),'g:');
%             legend
            ...
            ylim([-1 8]);
            xlabel('Frequency (Hz)'), ylabel('power(log) (\muV^2)');
            title(['Mean NREM power spectum and fitted linear SwiChR pre and post']);
            saveas(gcf, [outfolder, 'fig_avg.tif']);
             hold off
