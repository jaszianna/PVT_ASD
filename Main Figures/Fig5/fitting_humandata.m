...
fPath = 'g:\work\KOKI_mouse_sleep\data_humán\'; % set the path ot the input folder (which contains all individual spectum files in .dat)
outfolder = 'g:\work\KOKI_mouse_sleep\fitting\'; % output folder for figures and .xlsx files
addpath('g:\work\KOKI_mouse_sleep'); % set the forder where the script is located, or add script manually to the path (then, this line is not required)
electrodeidx = [11]; % set the number of electores
electrode_names = {'F4'}; % set electrode labels (required for the output only)
...
for loop = 1:length(List) % loop over each participant
    ...
    for elec = 1:numel(electrodeidx) % loop over each electrode
        ...
        elseif  isnan(spectra_nrem(1,elec))==1
            ...
            title('NREM power spectum and fitted linear elecrode F4');
            hold off
